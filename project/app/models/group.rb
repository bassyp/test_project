class Group < ApplicationRecord
  ...
  def get_all_users_as_string(*args)
    return User.where("uuid in (select user_uuid from user_join_groups where group_uuid = ?) and (parent_uuid is null or parent_uuid = '')", self.uuid).order(:last_name,:first_name).map{|e| ["#{e.last_name}, #{e.first_name} (#{e.id})"]}.join(", ")
  end
  ...
  def get_all_e_mails(*args)
    e_mails = User.where("uuid in (select user_uuid from user_join_groups where group_uuid = ?) and (parent_uuid is null or parent_uuid = '')", self.uuid).map{|e| "#{e.mail_address}"}
    e_mails = e_mails + "#{self.extra_e_mails}".split(/[,;]/)
    return e_mails.uniq.reject{ |e_mail| e_mail.blank? }
  rescue => e
    return []
  end
  ...
  def get_tree_array_active(*args)
    ...
    return {:id => "#{self.id};#{self.uuid}", :active => false, :position => self.position, :text => (self.active ? "#{self.name}" : "#{self.name} (deaktiviert)"), :population => nil, :checked => !self.active, :children => children}  
  end
