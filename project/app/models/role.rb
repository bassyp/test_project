class Role < ApplicationRecord
  ...
  def get_all_users_as_string(*args)
    User.where("uuid in (select user_uuid from user_join_roles where role_uuid = ?) and (parent_uuid is null or parent_uuid = '')", self.uuid).order(:last_name, :first_name).map { |e| [ "#{e.last_name}, #{e.first_name} (#{e.id})" ] }.join(", ")
  end
end
